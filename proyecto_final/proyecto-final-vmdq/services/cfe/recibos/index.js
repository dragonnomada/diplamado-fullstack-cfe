...
async function getEntregadosPagados(id, num_servicio, codigo_barras) {
	if (dbService.isConnected()) {
      const connection = dbService.getConnection()
			...
			let sql = "SELECT * FROM recibos WHERE entregado IS NOT NULL AND  pagado IS NOT "
			let params = []

			if (id) {
				sql = "SELECT * FROM recibos WHERE id = ? and entregado IS NOT NULL AND  pagado IS NOT"
				params = [id]
			} else if (num_servicio) {
				sql = "SELECT * FROM recibos WHERE num_servicio = ? and entregado IS NOT NULL AND  pagado IS NOT"
				params = [num_servicio]
			} else if (codigo_barras) {
				sql = "SELECT * FROM recibos WHERE codigo_barras = ? and entregado IS NOT NULL AND  pagado IS NOT"
				params = [codigo_barras]
			}
