...
async function getCancelados(id, num_servicio, codigo_barras) {
	if (dbService.isConnected()) {
      const connection = dbService.getConnection()
			...
			let sql = "SELECT * FROM recibos WHERE pagado IS NULL and cancelado IS NOT NULL"
			let params = []

			if (id) {
				sql = "SELECT * FROM recibos WHERE id = ? and pagado IS  NULL and cancelado IS NOT NULL"
				params = [id]
			} else if (num_servicio) {
				sql = "SELECT * FROM recibos WHERE num_servicio = ? and pagado IS NULL and cancelado IS NOT NULL"
				params = [num_servicio]
			} else if (codigo_barras) {
				sql = "SELECT * FROM recibos WHERE codigo_barras = ? and pagado IS  NULL and cancelado IS NOT NULL"
				params = [codigo_barras]
			}
	...
	if (dbService.isConnected()) {
      const connection = dbService.getConnection()
			...
			if (id) {
				...
				sql = "SELECT * FROM recibos WHERE codigo_barras = ? entregado IS NOT NULL"
				params = [codigo_barras]
			}
	...
	if (dbService.isConnected()) {
      const connection = dbService.getConnection()
			...
			if (id) {
				...
				sql = "SELECT * FROM recibos WHERE codigo_barras = ? entregado IS NOT NULL and pagado IS NOT NULL"
				params = [codigo_barras]
			}
...
async function getEntregadosPagadosCancelados(id, num_servicio, codigo_barras) {
	if (dbService.isConnected()) {
      const connection = dbService.getConnection()
			...
			if (id) {
				...
				sql = "SELECT * FROM recibos WHERE codigo_barras = ? entregado IS NOT NULL and pagado IS NOT NULL  and cancelado IS NOT NULL"
				params = [codigo_barras]
			}
	...
	if (dbService.isConnected()) {
      const connection = dbService.getConnection()
			...
			if (id) {
				...
				sql = "SELECT * FROM recibos WHERE codigo_barras = ? entregado IS NOT NULL  and cancelado IS NOT NULL"
				params = [codigo_barras]
			}
...
async function notificarRecibo(num_servicio, datos) {
	if (dbService.isConnected()) {
      const connection = dbService.getConnection()
			...
			let sql = "UPDATE recibos SET actualizado = NOW(), notificado = NOW(), firma_notificado = ? WHERE num_servicio = ?"
