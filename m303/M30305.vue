...
export default {
    ...
    watch: {
        edad(newVal) {
            if (newVal >= 90) {
                this.$refs.mensajeEdad.hidden = false
            } else {
                this.$refs.mensajeEdad.hidden = true
            }
        },
        bloqueado(newVal) {
            if (newVal) {
...
<template>
    <form ref="formulario" action="http://localhost:5000/api/confirmar" method="get">
        <div ref="mensajeEdad" style="color: red">
            <span>Tu edad es demasiado grande, será verificada por inspector</span>
        </div>
        <div>
