...
<template>
  <div>
	<h1>Victor Hugo Gonzalez Ahumada</h1><br/>
	<h2>Proyecto Final 3 - VUE Cliente / Servidor Integración</h2>
    <pre><code>{{ statusNuevoProducto }}</code></pre>
    <pre><code>{{ statusObtenerProductos }}</code></pre>
    <!--pre><code>{{ dataProductos || "null" }}</code></pre-->
    <button @click="fetchNuevoProducto('Hola',33.3)">Producto Nuevo</button>
    <button @click="fetchObtenerProductos()">Obtener Productos</button>
  </div>  
  <div>
    <NuevoProducto />
  </div>
